...
- Model-literal consolidation — DONE (`64a2ac2`, 2026-07-01): both generators declare a model constant + the 4 inline fetch-body literals reference it (next bump = one line per file); `VERIFY_MODEL` default now `claude-sonnet-5`. Still open: ±25-char citation-window mis-attribution (takes first year in range); guard-order decision (anti-cueing runs last in bulk — its drops can be masked).
- Dead scaffold removed (`963c523`, 2026-07-01): `netlify/functions/edge-functions/generate-mcq-stream.js` — created once via the GitHub web UI (2026-04-03), never wired to a Netlify build target (`netlify.toml` has no `edge_functions` directive; no root `netlify/edge-functions/` dir), so never built, routed, or served. Was on stale `claude-sonnet-4-5-20250929` but inert. 767-line delete; no other files touched.
