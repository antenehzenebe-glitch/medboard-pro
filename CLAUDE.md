...
> Last updated: **June 10, 2026** -- **v7.7-7.10 generator catch-up + full IM backlog triage (IM servable 241->320; bank 701->780).** This doc had drifted to v7.6.0; bringing it current. **Generator commits since v7.6.0:** v7.7.0 cross-run concept-saturation (`7881903`); v7.8.0 blueprint-proportional cap + deficit-weighted draw (`3a75868`); v7.9.0 blueprint-aligned TOPIC_DISTRIBUTION rebuild + Step-3 qTypePool + nutrition-injection retired (`2470070`,`0bf75c3`). **Patch A -- validator precision (`3636aa8`; both files, parity):** `flagCardiorenalMiskey` refactored to return `{hard,warn}` -- **H1 (HFrEF + SGLT2i offered + GLP-1 keyed) stays HARD-reject; H2 (SGLT2i<->hyperkalemia proximity) demoted to WARN** with a negation/contrast/MRA-attribution suppressor (kills the benign FP that silently dropped correct "SGLT2i are K-neutral / mitigate MRA hyperkalemia" items). Also tightened `flagSlidingScaleInsulin` to require sliding-scale<->insulin proximity, and reconciled the Step-3/IM/Endo `_lr` "FORBIDDEN most-likely-diagnosis" wording to "permitted at synthesis tier (~10-15%)" (bulk-only). **Patch B -- verify-pass (`69213a3`; bulk-only):** opt-in `VERIFY_PASS=1` independent blind re-answer (stem + shuffled choices only; dedicated `emit_answer` tool; temp 0); disagreement -> `_warnVerifyMiskey` warn; **fail-open, warn-only, lives in `runStandardMode` ONLY** -- default bulk mode is `batch`, so a verify run needs `--mode standard` + `VERIFY_PASS=1` or it silently no-ops. **IM backlog triage (2026-06-10):** all **91** IM `pending_review` rows dispositioned via two-pass review (lean projection -> full-text on flagged) -> **79 approved / 12 rejected**; IM servable **241->320** (past the ~165 flip target); pending 0; re_audit 0. Rejects = 5 genuine flaws (apixaban reload-dose at 3 wk; icosapent ethyl for TG-pancreatitis risk [fenofibrate is correct]; plasmapheresis keyed for a long-term-prevention question; clindamycin on a childhood amox rash [stale]; nicardipine-vs-clevidipine soft single-best) + 7 near-duplicate trims (HRS-AKI; ACO-dupilumab; CKD-patiromer; falls Z-drug; staph-PVE daptomycin; sarcoid-MTX; stroke-Aggrenox). **8 items rewritten in-place to guideline-sound instead of rejected:** HIV INSTI-myopathy premise + 1st/2nd-gen INSTI fix; MSSA-osteo clindamycin made single-best (D-test-negative + FQ-tendinopathy + K-5.0); cardiorenal H2 warn-isolation (above); and five today -- `0241013c` **re-keyed finerenone->dapagliflozin** (SGLT2i is the missing K-neutral 4th pillar in HFrEF+CKD+albuminuria; finerenone has no EF<40% outcome evidence, raises K, follows the SGLT2i), `bc003244` HFrEF stripped so midodrine (Class IIa) is clean for recurrent VVS, `a6e8cbf9` pregabalin->oxcarbazepine (kills the gabapentinoid-vs-gabapentinoid tie after duloxetine failure), `bff1f1c1` +blood-eosinophil 340 to justify ICS triple in GOLD Group E, `84272159` **re-keyed to resmetirom** (first FDA-approved F2-F3 MASH fibrosis agent) + family-history-of-MTC to exclude semaglutide. **Guard gap logged:** `0241013c` was a finerenone-over-SGLT2i mis-key that `flagCardiorenalMiskey` H1 missed (H1 only catches the GLP-1-over-SGLT2i variant) -- candidate **H3 (finerenone keyed over an offered SGLT2i in HFrEF)** for the next validator-precision patch. **Bank now 780 servable** -- IM 320 / Endo 201 / CK 102 / S3 80 / S1 77; all queues 0 pending; re_audit 0. **Open:** wire `VERIFY_PASS` into `bulk-generate.yml` env (Codespaces; MCP cannot write workflow paths) + run the first standard-mode verify batch; build H3; resume staged USMLE generation under B4/B5.
>
> Last updated: **June 9, 2026** — **Permanent diversity + clinical-correctness guards SHIPPED (commit `cb4d40d`); legacy `S1/S2/S3` explanation backlog CLEARED; USMLE +54 servable.** **Layer 1 `flagConceptSaturation`** (bulk-only, HARD-REJECT, ≤1/concept): the long-deferred §12 "B4 semantic-dedup" finally realized as a *blocking* guard. Root cause of the recurring clusters was that B4 was **warn-only** + **stem-only** (so template-sharing-but-distinct items couldn't be safely blocked); the fix adds the **key dimension** — a true duplicate = stem B4-similar **AND** keyed-answer ≥0.40 token-contained vs a prior accepted item this batch. Validated on the real batches: osteomalacia 4→1, cardiorenal pair 2→1, ICU/sepsis twins→1, **zero false-positives** (CKD-nutrition's 4 distinct interventions + EOL's distinct principles spared); honest residual = synonym-divergent phrasings of one mechanism survive (Wernicke 5→3 — lexical containment can't equate "acetyl-CoA" with "α-ketoglutarate dehydrogenase"). Threshold-robust (0.30–0.50 identical). **Layer 2 (both files, parity, warn-mode):** `flagMetforminEgfr` (the eGFR-32-"below the threshold of 30" misstatement + stop-metformin-at-30–45), `flagSlidingScaleInsulin` (anti-pattern; heightened in dementia/elderly), `flagGdmCoherence` (male/no-pregnancy GDM topic-drift). **`flagT1DCardiorenal` (warn-mode, both files, parity)** shipped earlier this session (T1D-keyed finerenone/SGLT2i/GLP-1; islet-Ab-**positivity** gate + negation + hazard-framing suppressors; 12/12 synthetic + 0/23 backtest FP) — **still un-exercised** (no T1D content in the 90-question run), so the warn→hard-reject promotion stays ON HOLD until a batch actually tests it (vacuous-clean batches do not count). **Legacy explanation backlog CLEARED:** true count was **104** (not the logged 99 — the parenthetical `S1 (Correct answer):` form had evaded the audit regex; authoritative detector is now ``(^|\n)[ \t]*\*{0,2}[ \t]*S[123][ \t]*(\([^)]*\))?[ \t]*[—–:-]``), all relabeled in-place to the 🩺/🚫/🔍/💎 emoji format (header-only, lossless; the `S1 (...)` evasion + one orphaned `**` typo fixed in the same pass); generators confirmed emoji-only so it stays at zero (S3 competing-dx → 🔍, a 4th marker on 23 rows, won't trip `validateChoiceCompleteness` which runs at generation). **DB hygiene:** IM lipid labels consolidated (a **4-way** split, not 3) → canonical `Lipid Disorders`; `8d3a3fe0` re-topiced to ethics/EOL; 26 `approved`/NULL `approval_status` rows normalized. **USMLE 90-question staged run** (Step 1/2CK/3, 30 each, pre-guard) triaged → **+54 servable at a ~60% post-triage yield** — reject driver = conceptual near-dups (Wernicke ×5, osteomalacia ×4, ICU/sepsis/cardiorenal twins) + mis-keys (metformin-eGFR-32 ×2, sliding-scale-in-dementia, an EOL verbal-revocation-over-written-AD mis-key, GDM-in-a-man ×2) — the exact classes the new guards now block/flag. **Servable 561→615** (Endo 201 / IM 175 / Step 2 CK 96 / Step 1 72 / Step 3 71); **pending 0; `re_audit` 0; rejected up by the triage rejects.** Generators now **v7.6.0**. Open: bump bulk `main()` banner to v7.6.0 at next commit; **phase-2** = extend concept-dedup to compare against the *existing bank* (cross-run), not just intra-batch (prefetch banked keys per topic, like B5 does for counts).
>
> Last updated: **June 7, 2026** — **B2 topic-consistency parity, B5 generation cap, and cardiorenal warn→hard-reject ALL SHIPPED.** **B2** (`flagTopicMismatch`) mirrored into `generate-mcq.js` — hard-reject gates `isValid` in both chains; pediatric-age = PART-2d warn (commit `5f18f31`). **B5** per-(exam_level,topic) generation cap (bulk-only): budget = max(0, ceiling − servable+pending), redistributes freed slots, exempts catch-alls; env `GEN_CAP_CEILING` (default 8) / `GEN_CAP_EXEMPT` — closes the cross-batch saturation gap B4 can't see. **Cardiorenal promoted warn→hard-reject (variant: fixed-both)** in both files (parity), and a latent bug fixed: H1 resolved the key from `p.correct_answer` (unset at validation) so it had been a **no-op** — now reads `p.correct`, plus a weight/glycemia tie-break so a GLP-1 legitimately keyed for weight loss in an HFrEF patient is not rejected (B5 + cardiorenal in commit `94e83b2`). `flagCardiorenalMiskey` verified byte-identical across both files post-deploy (parity OK). **Bank (2026-06-07):** 8 stale `re_audit` rows finalized→`rejected` (EOL capacity-autonomy trio + 34–38wk preeclampsia/HELLP quintet); a saturated-topic read demoted 5 unambiguous near-dups (Step 1 DKA-K + severe-AS; Step 2 CK ×2 NSTEMI-dx; Step 3 inferior-STEMI non-PCI) which were then **finalized→`rejected`**; protected metabolic trio (AKA `0c08d131` / HHS `aa31e850` / MALA `db673c2e`) untouched. **Servable 522→561** (Endo 201 / IM 175 / Step 2 CK 70 / Step 1 64 / Step 3 51); **`re_audit` = 0; rejected = 202.** Open hygiene: the one-off deploy `.py` scripts were committed into the repo (rm in a chore commit); B5 + cardiorenal share one commit (`94e83b2`) rather than two; bulk `main()` banner prints v7.5.15 vs file header v7.5.16 (cosmetic — bump on next substantive bulk change). Findings: IM lipid-label fragmentation (`Lipid di` / `Lipid Disorder` / `Lipid Disorders` = one topic split 3 ways); `8d3a3fe0` is an ethics/surrogate item mis-topiced under Step 2 CK "Sepsis and Septic Shock" (re-topic candidate).
>
> Last updated: **June 6, 2026** — **Cardiorenal SGLT2i-deprioritization guardrail shipped** (commits `c260ad1` L2 + `4e7e0e2` L1): L1 `TOPIC_GUARDRAILS` anchor (SGLT2i = Class I for HFrEF, initiated to eGFR 20, potassium-neutral; cardiorenal indications T2D-specific) + `flagCardiorenalMiskey` warn-mode validator (H1 HFrEF-keys-GLP-1-over-SGLT2i; H2 false-hyperkalemia rationale), both files parity, no banner bump. Backtest recall 3/3; approved-bank FP 0 H1 / 1 benign H2. **2026-06-06 full Endo triage:** fresh batch (14, guardrail-live, 0 cardiorenal warns) + backlog (37) cleared across 3 tranches — 30 promoted, 17 rejected (2 cardiorenal mis-keys, soft single-bests, near-dups, phantom-citation/clinical-error items); **Endo servable 163→201, pending → 0, ~175 flip target MET**; total 346→406. **Then (same day):** IM bulk run (28, B3-spread) fully triaged → IM 148→175 (**~165 flip target MET**), 1 reject; full 34-row USMLE pending backlog triaged → Step 1 22→36, Step 2 CK 16→27, Step 3 19→28. **Total 406→467; 0 pending bank-wide — every queue empty.** Cardiorenal guardrail clean across the IM + USMLE batches (0 mis-keys; correct SGLT2i keying in `1cd0de2d`/`355a9d1d`/`d99b0afe`) — ≥2 clean batches now on record. **USMLE generation stretch (2026-06-06):** per-level bulk runs for Step 1/2CK/3 (26+29+30 = 85 generated) triaged → 55 promoted, 30 rejected (~35%; Step 3 batch hit 50%); Step 1 36→54, Step 2 CK 27→49, Step 3 28→43; **total 467→522, 0 pending bank-wide.** Reject driver = heavy *within-topic* semantic near-duplication (B3 spreads across topics, not within) + repeated stem-opener templates + a few manual-only catches (HF CRT-at-6-wk contradiction, Pap co-test mis-key, preeclampsia transfer-vs-deliver, GDM-in-a-man mis-topic, X-ALD enlargement error, PE fondaparinux/HIT). **Proposed fix: B4 semantic-dedup pre-filter + intra-batch concept ledger + verify-pass — see §12 "USMLE generation hardening."** **Prior (June 5) — v7.5.16:** Endocrine Society 2012 added to citation allow-list (real ES MEN1 CPG — Thakker, JCEM 2012); **v7.5.15:** interchangeable-agent validator tuning. Endo tail-cluster triage (CGM/AID, Hashimoto's, Male Hypogonadism, thyroid/Ca): 14 promoted (incl. 5 citation/terminology repoints — 3 phantom "2024 ES" survivors purged), 4 rejected (T1D-finerenone, Klinefelter-mislabel, sorafenib-renal-error, SGLT2i-K⁺-error); Endo 110→141, total 315→346. **Prior (June 4):** Endo pending-triage: 6 clusters dispositioned (GLP-1, Cushing, PCOS, Type 1 Insulin, Hyperthyroid/Graves, Osteoporosis); Endo 84→110 servable, total 289→315. **SURPASS-CVOT (NEJM 2025) clinical-canon shift** recorded (tirzepatide CV data — "lacks CVOT" items now stale; servable `6cf8c36d` fixed). Prior (June 3): Pituitary Society 2023 added to the citation lock; §4 threshold table repaired. **v7.5.12 (2026-06-02):** PA citation repointed to ES 2025 (Adler) + `validateNoPhantomCitations` phantom-citation hard block added. Prior (May 31): B3 topic-distribution control shipped; D1 closed; citation lock complete. **Lipid canon refreshed in place:** lipid `TOPIC_GUARDRAILS` + citation-map rewritten to the **2026 ACC/AHA/Multisociety Dyslipidemia Guideline + AACE 2025**; the **2018 Grundy citation retired**.
...
| ABIM Internal Medicine | 320 | ~165 | **met** |
| USMLE Step 1 | 77 | ~150 | ~73 |
| USMLE Step 2 CK | 102 | ~180 | ~78 |
| USMLE Step 3 | 80 | ~150 | ~70 |
| **Total servable** | **780** | | |

> 467 = `status='approved' AND cueing_flag IS NOT TRUE` = servable. Verified live 2026-06-06. **2026-06-06:** Endo 141→201 servable (fresh batch of 14 + full backlog triage of 37 across 3 tranches), total 346→406; **Endo pending 41→0 — backlog fully cleared.** **ABIM Endocrinology ~175 flip target MET (201).** Cardiorenal guardrail shipped; first live read clean (0 warns on the fresh batch); 17 backlog rows rejected (2 cardiorenal mis-keys, soft single-bests, near-dups, phantom-citation/clinical-error items). **2026-06-06 (later same day):** IM bulk run of 28 (B3-spread) triaged → IM 148→175 (**~165 flip target MET**, 1 reject); full 34-row USMLE backlog triaged → Step 1 22→36, Step 2 CK 16→27, Step 3 19→28; total 406→467. **All five queues now empty (0 pending bank-wide).** `re_audit` cohort empty. **2026-06-06 USMLE generation:** per-level bulk runs (Step 1/2CK/3, 85 generated) triaged → 55 promoted / 30 rejected; Step 1 36→54, Step 2 CK 27→49, Step 3 28→43; total 467→522, all queues re-cleared to 0. Reject driver = within-topic semantic near-duplication — see §12 "USMLE generation hardening."

Pending (not servable until vetted): **0 bank-wide.** **2026-06-07:** servable 522→561 after a saturated-topic trim and finalizing the `re_audit` cohort — 8 stale rows + the 5 trim near-dups all → `rejected`. **`re_audit` = 0; rejected = 202.** Endo 0, IM 0, Step 1 0, Step 2 CK 0, Step 3 0. Triage rule in force: **keep ≤2 distinct sub-angles per concept.** Two heavily-seen reject classes: (a) "two co-valid agents" soft single-best (see §12) and (b) **mis-keyed** items where the labs/organ/guideline-rank contradict the keyed answer (need a human clinical read; no validator catches them — `flagCardiorenalMiskey` now warn-flags the SGLT2i/HFrEF subclass).

### Generators
Generators at **v7.5.16** (2026-06-05; +ES-2012 citation allow-list, +`flagInterchangeableAgents` tuning), **+cardiorenal SGLT2i guardrail (2026-06-06: L1 anchor + `flagCardiorenalMiskey` warn-mode, no banner bump)**, prior **v7.5.13** (2026-06-03; +2a lead-in allow-list, +2b choices minLength). **2026-06-09 — v7.6.0:** +`flagConceptSaturation` (Layer 1 concept-dedup, bulk, hard-reject) + `flagT1DCardiorenal` + `flagMetforminEgfr`/`flagSlidingScaleInsulin`/`flagGdmCoherence` (Layer 2 warns, both files, parity) — commit `cb4d40d`. B3 shipped + smoke-confirmed. Citation lock complete. C1 (drop-reason breakdown) + C2 (`generation_model` per-row) shipped. **Generation outage fixed** (`BULK_CLAUDE_MODEL` use-before-declare → declared at module scope, line 68). Error-surfacing patch live (catch blocks log HTTP status + body + per-attempt cause) — keep it.
...
2. **IM pending-triage — COMPLETE 2026-06-06** (servable 175; pending 0; ~165 flip target MET). 28-question B3 batch triaged (27 promoted incl. a renal-anticoag contradiction-pair reconciliation; 1 reject). Next: deepen via further moderate IM batches.
3. **USMLE — building toward threshold.** Step 1 54 / Step 2 CK 49 / Step 3 43 (targets ~150–180); all queues empty 2026-06-06. Three per-level batches (85 generated) added 55 after a ~35% dedup-driven reject (Step 3 batch was 50%). **BLOCKER before scaling USMLE generation further: build B4 semantic-dedup + verify-pass** (see §12 "USMLE generation hardening") — otherwise every batch needs a heavy manual cluster-trim. After B4 ships, resume staged per-level generation (count 20–30, vet each batch).
...
7. **Promote `flagCardiorenalMiskey` to hard-reject — SHIPPED 2026-06-07 (commit `94e83b2`, variant fixed-both).** Found H1 had been a **no-op** (resolved the key from `p.correct_answer`, unset at validation) — fixed to read `p.correct`, added a weight/glycemia tie-break so a GLP-1 RA legitimately keyed for weight loss in an HFrEF patient is not rejected, and promoted both rules to hard-reject (bulk `recordDrop('_cardiorenalRejected')`; gen-mcq `cardiorenalOk` in both `isValid` chains; function byte-identical across files). The prior ≥2-clean-batch evidence really only validated H2 (the 2026-06-06 IM + USMLE batches produced 0 cardiorenal warns, with correct SGLT2i keying in `1cd0de2d`/`355a9d1d`/`d99b0afe`). Promotion to hard-reject can now be considered. Also build sibling `flagT1DCardiorenal`.
...
A clinically-wrong-but-validator-invisible key class, adjacent to the T1D cardiorenal-extrapolation note (§9): the generator keys a GLP-1 RA over an available SGLT2i in an HFrEF/CKD stem, justified by a **false** "SGLT2i cause/worsen hyperkalemia" claim or the **irrelevant** "SGLT2i lose glycemic efficacy below eGFR 45" argument. In HFrEF, SGLT2i is **Class I** (EMPEROR-Reduced/DAPA-HF), initiated to eGFR 20, regardless of glucose; GLP-1 RAs reduce ASCVD events but **not** HF hospitalization; SGLT2i are potassium-neutral to K-lowering. **`flagCardiorenalMiskey`** (warn-mode, both paths; mirrors `flagInterchangeableAgents`, never drops): **H1** HFrEF stem + an SGLT2i offered + the keyed answer is a GLP-1 RA; **H2** `SGLT2[^.]{0,60}hyperkalem` proximity in the explanation. Warns print via `console.warn` on both paths; bulk also increments `dropTally._warnCardiorenal`. **L1 generation anchor** added to the T2D management guardrail (the rate-reducer). Backtest 2026-06-06 (whole approved bank + node unit test): recall 3/3 (`ec94b12a`, `c6714248` → H1; `12f5f085` → H2); approved-bank false positives **0 (H1) + 1 benign (H2)** — `32278205`, a correct MRA-attribution sentence (expected warn noise). **First live read (2026-06-06 fresh Endo batch of 14): 0 warns** — the L1 anchor steered generation away from the mis-key. **Subsequent IM and USMLE batches (same day) also produced 0 cardiorenal warns**, with correct SGLT2i keying in HFrEF/CKD stems (`1cd0de2d`, `355a9d1d`, `d99b0afe`) — **≥2 clean batches now on record**, sufficient to consider the warn→reject promotion (§9 item 7). Sibling `flagT1DCardiorenal` (finerenone/SGLT2i/GLP-1 keyed in a T1D stem) is the next candidate. The 2 pre-guardrail backlog instances `ccae9922` and `dd4e9fb4` were **rejected 2026-06-06**.
...
### USMLE generation hardening (B4 SHIPPED 2026-06-06; B5 cap SHIPPED 2026-06-07; original 2026-06-06 analysis below)
Three per-level USMLE batches (Step 1/2CK/3, 85 items) ran a ~35% reject — the Step 3 batch hit 50%. Root cause: **B3 spreads across topics but the model duplicates *within* a topic** (e.g., 6 near-identical CKD-nutrition stems, 3 acyclovir-MOA, 5 thiamine/Wernicke, 4 vitamin-D osteomalacia), plus repeated stem-opener templates ("found unresponsive at home," "brought to the ED after ingesting," identical age/sex), plus a handful of defects only manual clinical read caught (mis-keys, contradiction-pairs, mis-topic). No current validator checks semantic novelty or answer-correctness — they check format, lead-in type, citations, and the cardiorenal/interchangeable patterns only. **Proposed four-layer fix, in priority order:**

1. **B4 — semantic near-dup pre-filter (highest leverage).** Before saving a generated MCQ, compare its stem against existing approved + pending rows of the SAME `(exam_level, topic)`; drop or flag if too similar. *Full version:* embed the stem (add a pgvector `stem_embedding` column, backfill the existing bank once) and drop on cosine similarity ≥ ~0.88 (calibrate); warn-mode first (`dropTally._warnSemanticDup`), promote to hard-drop after threshold tuning. *Cheap interim (no embeddings):* MinHash/SimHash shingling on the stem, OR a `concept_signature` = (topic, normalized keyed-answer, primary clinical anchor) with drop-on-exact-match. Either interim would have auto-caught the 6 CKD-nutrition / 3 acyclovir / 4 vitamin-D clusters this session.
2. **Intra-batch concept-coverage ledger (generation-side).** Extend the B3 bulk loop: per topic, accumulate the sub-angles / keyed-concepts already generated *this batch* and feed them into the next same-topic prompt — "you already generated X and Y for this topic; produce a DISTINCT sub-angle or skip." Pair with a per-topic **sub-angle taxonomy** in `TOPIC_GUARDRAILS` encoding the ≤2 canonical angles, and require the model to tag each item's sub-angle.
3. **Stem-template diversification (prompt rule).** Ban the repeated opening templates and require varied age/sex/clinical setting; no two items in a batch may share an opening sentence. (Manual opener-diversification was needed in every USMLE batch.)
4. **Verify-pass (warn-mode — catches the manual-only defect classes).** After generation: (a) **self-consistency** — independently answer the question with a second model pass and flag if it disagrees with the keyed answer (catches mis-keys: the HF-CRT-at-6-weeks, Pap co-test, preeclampsia transfer-vs-deliver). (b) **topic-consistency rules** — e.g., a "Gestational Diabetes" stem must contain pregnancy; "Pediatrics" implies age <18; flag mismatches (catches the GDM-in-a-man and the mis-topiced "Obesity pharmacotherapy"/IFG item). (c) B4 surfacing a "near-identical stem with a *different* keyed answer" = automatic contradiction-pair flag.

**Recommended build order: B4 (layer 1) + verify-pass self-consistency (layer 4a) FIRST** — together they auto-catch the large majority of the 30 rejects; layers 2–3 then cut upstream generation waste. Implement on `scripts/bulk-generate.js` first (the batch path where clustering happens), then mirror the dedup/verify hooks to `netlify/functions/generate-mcq.js` for parity — noting the B-series sampler logic remains bulk-only by design (the live single-question path doesn't batch-sample). Until B4 ships, every USMLE batch requires a manual ≤2-per-concept cluster-trim + opener diversification (the 2026-06-06 workflow).
...
### 2026-06-07 session (guards shipped + bank trim)
- **B2 topic-consistency guard — parity SHIPPED (`5f18f31`).** `flagTopicMismatch` mirrored into `generate-mcq.js`; hard-reject gates `isValid` in both the primary and Gemini-fallback chains; pediatric-age is a non-blocking PART-2d warn. Byte-identical across files.
- **B5 generation cap — SHIPPED (bulk-only; in `94e83b2`).** Complements B4: B4 dedupes *within* a batch; B5 stops *cross-batch* saturation. `async fetchTopicBudgets()` counts servable+pending per `(exam_level, topic)`; budget = `max(0, GEN_CAP_CEILING − count)` (registers every distribution topic so zero-count topics are also capped); `buildWorkQueue(count, budgets)` round-robins only eligible topics, redistributing freed slots; exempt topics (default `Random -- All Specialties`) absorb overflow. Env: `GEN_CAP_CEILING` (default **8**; ≤0 disables), `GEN_CAP_EXEMPT` (pipe-delimited). **Policy knob:** at ceiling 8, broad Endo topics already >8 (`Type 2` 15, `Hypoglycemia` 11) get 0 new — raise the ceiling or exempt to deepen those. Bulk-only by design (single-question path isn't batch-sampled — same exemption as B3/B4).
- **Cardiorenal warn→hard-reject + H1 fix — SHIPPED (`94e83b2`, variant fixed-both).** See §12 "SGLT2i-deprioritization mis-key" and §9 item 7. H1 was a no-op (key from `p.correct_answer`, unset at validation), so the "≥2 clean batches" only ever validated H2; H1 is now live (key from `p.correct`, + weight/glycemia tie-break) and both rules hard-reject. Verified parity post-deploy.
- **Bank trim/finalize.** 8 stale `re_audit` rows → `rejected` (EOL capacity-autonomy trio; 34–38wk severe-preeclampsia/HELLP quintet — distinct OB items + a 32-wk HELLP retained). Saturated-topic read demoted then **finalized→rejected** 5 unambiguous near-dups: `574a28fa` (DKA-K dup of `fa3582cd`), `76cc58fd` (severe-AS dup of `cbe64a2e`), `1bf1f6e2`+`2aa5d6db` (NSTEMI-dx dups of `cff8a7ff`), `030be69b` (inferior-STEMI non-PCI dup of `5e9d8152`). Protected metabolic trio AKA `0c08d131` / HHS `aa31e850` / MALA `db673c2e` untouched. Deliberately kept (different keyed answers ⇒ likely distinct): Physiology metabolic-alkalosis-vomiting pair; Step 2 CK + Step 3 obstructive-uropathy sepsis sets. **Servable 561; `re_audit` 0; rejected 202.**
- **Repo hygiene (open).** One-off deploy `.py` scripts (`deploy_b5_cap_bulk.py`, `deploy_cardiorenal_hardreject.py`, etc.) were committed into the repo — remove in a `chore:` commit (convention is rm-after-run). B5 + cardiorenal landed in a single commit (`94e83b2`) rather than two single-purpose commits. Bulk `main()` banner prints `v7.5.15` while the file header is `v7.5.16` — cosmetic; bump on the next substantive bulk change.
- **IM lipid-label fragmentation (data hygiene).** `Lipid di` (5) + `Lipid Disorder` (5) + `Lipid Disorders` (5) are one topic split across three label strings — consolidate to one canonical label (separate from the cap, which keys on exact strings).
- **Mis-topiced row.** `8d3a3fe0` is an ethics/surrogate-decision item filed under Step 2 CK "Sepsis and Septic Shock" — re-topic candidate (not a trim).
